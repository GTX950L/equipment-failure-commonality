...
>
> 当前版本 **v3.8.6** · 单文件 HTML，浏览器即开即用（含 Wilson 置信区间 / Fisher 显著性 / 时间演化）
...
4. 一键出图 → 悬停节点看「NG 占比 / 95% 置信区间 / 与整体是否显著偏离」，点节点自动筛选 + CPK

**出图能力**：
- 每层标注字段名（X 轴层标题）；节点 hover 显示 NG 占比、样本数、完整取值、**95% 置信区间**、**与整体基线的显著性（Fisher p 值）**——红色是"真红"还是样本太少虚高，一目了然
- **点击任意节点**：该路径红色粗边框高亮，下方 CPK 面板自动对该子集做能力分析
- **⏱ 时间演化**：数据含时间列时，拖动滑条 / 一键播放，看 NG 根因从哪个时刻开始出现、何时恶化
- **双图对比**：同工序配置下并排对比两组（如夜班 vs 白班）的红色路径差异
- 配色可切换（科学渐变 / JMP 红蓝二色）、导出 PNG / SVG 矢量图、根因报告一键汇总
- 小样本保护：n&lt;5 的节点置灰 + 悬停警示「颜色仅供参考」，避免把偶然当根因
- 页面右上角内置 **📖 排查指南**：FACA 五步法 + 三个必避陷阱 + 控制变量法 + 排查完成清单
...
**统计证据（悬停节点直接可见）**：每个节点还带 **95% 置信区间（Wilson）** 与 **Fisher 精确检验**——
`注水阀=2 → 显著偏高 (p=7.9e-5)`、`注水量<0.1048 → 显著偏高 (p=1.2e-11)` 这类结果直接告诉你
"这个档位的 NG 偏高不是巧合"，与 CPK 判级互相印证，结论更经得起评审。
...
- [x] 统计推断 (Wilson 95% 置信区间 + Fisher 显著性, 小样本保护)
- [x] 时间演化 (滑条/播放看 NG 率随时间变化) + 双图对比
- [x] 小屏体验 (面板折叠 / 参数搜索 / 出图按钮粘性) + 性能优化 (引擎懒加载)
